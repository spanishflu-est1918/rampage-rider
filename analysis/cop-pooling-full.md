...
Here are the complete code replacements for `src/entities/Cop.ts` and `src/managers/CopManager.ts` implementing the object pooling pattern.

### 1. src/entities/Cop.ts

Added `reset()` and `deactivate()` methods, and updated state management for pooling.
...
export class Cop extends THREE.Group {
  ...
  /**
   * Reset cop state for reuse (pooling)
   */
  reset(position: THREE.Vector3): void {
    ...
    // Re-add to manager if it was removed
    this.yukaEntityManager.add(this.yukaVehicle);
  ...
  /**
   * Deactivate cop for pooling (disable physics/ai/visuals)
   */
  deactivate(): void {
    this.visible = false;
...
### 2. src/managers/CopManager.ts

Added `copPool`, updated `spawnCop` to use pool, and `updateSpawns` to pool dead cops instead of disposing them.
...
export class CopManager {
  private cops: Cop[] = [];
  private copPool: Cop[] = []; // Pool for reusing dead cops
  ...
  updateSpawns(heat: number, playerPosition: THREE.Vector3): void {
    ...
    for (let i = this.cops.length - 1; i >= 0; i--) {
      ...
      if (cop.isDeadState() && !(cop as THREE.Group).visible) {
        // Cleanup dead cop and return to pool
        this.scene.remove(cop);
        this.scene.remove(cop.getBlobShadow());
        cop.deactivate(); // Reset state and remove from AI/Physics activity
